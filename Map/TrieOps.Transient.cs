    ...
    IEqualityComparer<TK> comparer, ulong ownerId, out bool added)
{
    ...
    if (flags == NodeFlags.Internal)
    {
        ...
        if ((dataMap & bitpos) != 0)
        {
            ...
            byte childCapCurrent = NodeOps.GetCapacity(node.Meta);
            var newNodeObj = NodeOps.AllocateInternal<TK, TV>((byte)(childCapCurrent + 1), NodeFlags.Internal, ownerId, newMap);
        ...
        byte childCapEmpty = NodeOps.GetCapacity(node.Meta);
        var appendedNodeObj = NodeOps.AllocateInternal<TK, TV>(childCapEmpty, NodeFlags.Internal, ownerId, insertMap);
    ...
    if ((dataMap & bitpos) != 0)
    {
        ...
        if (comparer.Equals(existingSlot.Key, key))
        {
            ...
            byte leafCap = NodeOps.GetCapacity(node.Meta);
    ...
    added = true;
    byte currentLeafCap = NodeOps.GetCapacity(node.Meta);
    var expandedLeaf = NodeOps.AllocateLeaf<TK, TV>((byte)(currentLeafCap + 1), NodeFlags.None, ownerId, node.Map | bitpos);
